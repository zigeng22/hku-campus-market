...
| AUTH-02 | IN PROGRESS | 实现 SessionManager 与 MainActivity 路由 | ARCH-02 | 2026-07-22 `fix/p0-session-search-stability` 已在启动时通过 Repository 验证 session userId，失效 ID 自动清除并进入 Login；2 个路由测试及最终全量 22 个仪器测试在 Medium_Phone API 36 通过，待 PR 合并后改为 DONE |
...
| ITEM-03 | IN PROGRESS | 实现 ACTIVE 搜索、seller listings 和 ItemCard 查询 | ITEM-01 | 2026-07-22 `fix/p0-session-search-stability` 已将关键字扩展到 name/description，并为价格和时间排序增加稳定 ID 次级顺序；4 个 SQLite 搜索测试及最终全量 22 个仪器测试在 Medium_Phone API 36 通过，待 PR 合并后改为 DONE |
...
| ITEM-06 | IN PROGRESS | 实现相册选择和 URI 持久化 | ITEM-05 | 2026-07-22 `fix/p0-session-search-stability` 已统一持久读取权限异常、空/失效/不可读 URI 回退；发布预览、首页卡片和详情页使用项目占位图，3 个 URI 加载测试及最终全量 22 个仪器测试在 Medium_Phone API 36 通过，待 PR 合并后改为 DONE |
| ITEM-07 | IN PROGRESS | 实现 ItemDetailActivity 身份视图 | UI-05, ITEM-02 | seller/buyer action、编辑、删除和 offer 导航已连接；待状态与设备验收 |
| TEST-ITEM-01 | IN PROGRESS | 商品 Repository 和金额/搜索测试 | ITEM-02, ITEM-03 | `MarketRepositoryItemSearchTest` 已覆盖 name/description、大小写、分类、ACTIVE、空关键字和稳定排序；CRUD、owner 权限和价格边界仍待补齐 |
...
3. `AUTH-02`：失效 session userId 清理和 2 个路由测试已在当前集成分支完成，待 PR 合并。
4. `ITEM-03`：description 搜索、稳定排序和 4 个查询测试已在当前集成分支完成，待 PR 合并。
5. `ITEM-06`：图片 URI 失效/权限异常回退和 3 个加载测试已在当前集成分支完成，待 PR 合并。
