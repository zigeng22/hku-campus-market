...
| TRADE-03 | IN PROGRESS | 在 ItemDetail 连接 Offer Dialog/Buy Now | UI-06, TRADE-02 | 2026-07-22 `fix/TRADE-03-buy-now-ui` 已将 Buy Now 改为固定标价确认流程，Make Offer 保留金额输入和字段错误；2 个 UI 流程测试及全量 13 个仪器测试在 Medium_Phone API 36 通过，待 PR 审核合并后改为 DONE |
...
2. `TRADE-02/TEST-TRADE-01`：PR #4 已合并并更新为 `DONE`；当前执行 `TRADE-03`，修正 Buy Now UI 流程。
3. `AUTH-02`：补失效 session userId 的启动清理。
4. `ITEM-03`：补 description 搜索与查询测试。
