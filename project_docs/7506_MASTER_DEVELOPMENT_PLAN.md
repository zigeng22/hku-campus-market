...
这一节是多人能否顺利合并的关键。`CONTRACT-01` 合并后即冻结；任何修改都必须由明确任务授权，并同时更新调用方、测试和本文件。契约版本 1.1 于 2026-07-22 经 `TRADE-06/CONTRACT-01` 授权，将 `ItemCard.status/offerCount` 正式纳入必填字段；Repository 公共方法签名未变化。
...
| DEMO-01 | IN PROGRESS | Seed demo data 或稳定测试账号方案 | INT-01 | `feat/demo-data-setup` 增加仅 Debug 可见的一键准备按钮，幂等创建 Alice/Bob、Pending 和 confirmed 场景；Debug/Release 构建、46 个仪器测试及设备操作通过，待 PR 合并 |
...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| DOC-01 | IN PROGRESS | README | INT-01 | `feat/demo-data-setup` 已新增根目录 README，覆盖项目功能、环境、构建运行、架构、演示账号、本地数据、测试、限制和依赖说明；待 PR 合并及最终 attribution 复核 |
...
2. `DEMO-01`：当前 `feat/demo-data-setup` 已完成仅 Debug 可见的一键演示数据、固定 Alice/Bob 账号和使用说明，待 PR 合并。
3. 下一批执行 `UI-08/QA-01`：只检查主要手机尺寸和第 14 节 P0 手工矩阵，修复实际可见问题。
4. `DOC-01`：当前分支已新增根目录 README，待 PR 合并和最终 attribution 复核。
5. `DEMO-01/DOC-01` 合并后再处理报告、视频脚本和最终提交材料，不新增后台、云服务或商用级功能。
