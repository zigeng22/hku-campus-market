...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| INT-01 | IN PROGRESS | 从干净安装跑 Alice/Bob 完整主流程 | Phase 1-3 | `feat/p0-e2e-stabilization` 已在 API 36 从清空数据完成注册、发布、普通报价、Pending、卖家接受、首页下架、Deal confirmed/联系人及重启持久化；待 PR 合并后改为 DONE |
| INT-02 | IN PROGRESS | 修复 Activity 生命周期、返回栈和 onResume 刷新 | INT-01 | API 36 已验证成交后 Offer Review、Item Detail、My Listings、Home 逐层返回均自动刷新，旧详情不再显示购买/卖家操作；待 PR 合并 |
| INT-03 | IN PROGRESS | 统一错误、空状态和 Snackbar/Dialog 行为 | INT-01 | `feat/p0-e2e-stabilization` 补齐类别下拉稳定展开和成交状态 `Deal confirmed`；登录/注册、商品、报价、管理中心普通流程反馈已统一，待 PR 合并 |
| UI-08 | TODO | 视觉一致性和响应式检查 | INT-01 | 两种手机尺寸截图无重叠、截断、不可点击控件 |
| QA-01 | TODO | 全量手工测试矩阵 | INT-01 | 第 14 节所有 P0 用例有 Pass/Fail 与修复链接 |
| QA-02 | IN PROGRESS | 单元/仪器测试回归和完整构建 | QA-01 | `feat/p0-e2e-stabilization` 已完成 assembleDebug、testDebugUnitTest、lintDebug 和 API 36 connectedDebugAndroidTest；45/45 仪器测试通过，待 PR 合并 |
...
1. PR #3 至 #9 的认证、商品、报价、成交、schema、全部 P0 页面和管理中心任务已合并；对应任务已更新为 `DONE`。
2. `INT-01/INT-02/INT-03`：当前 `feat/p0-e2e-stabilization` 已完成 API 36 Alice/Bob 主流程和返回刷新验收，修复类别下拉并将成交状态显示为 `Deal confirmed`，待 PR 合并。
3. `QA-02`：当前分支的构建、Lint、单元测试和 45 个仪器测试全部通过，待 PR 合并。
4. 下一批执行 `UI-08/QA-01`：只检查主要手机尺寸和第 14 节 P0 手工矩阵，修复实际可见问题。
5. `DEMO-01/DOC-01`：整理稳定演示账号准备方法和 README，不新增后台、云服务或商用级功能。
