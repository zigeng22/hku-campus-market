...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| UI-06 | IN PROGRESS | Make Offer Dialog 与 row_offer XML | UI-01, UI-05 | `feat/ui-management-flows` 已完成 HKD 金额输入、报价类型/日期/状态和 Pending Accept 布局；编译、Lint 与 API 36 全量 45 个仪器测试通过，待 PR 合并 |
| UI-07 | IN PROGRESS | Management/Listing/Participation layouts | UI-01, CONTRACT-02 | `feat/ui-management-flows` 已完成两个 Tab、三种 row、分 Tab 空状态、商品图与成交联系方式布局；1080x2400 空状态实机截图通过，待 PR 合并 |
...
| TRADE-04 | IN PROGRESS | 实现 seller offer list 查询与 OfferReviewActivity | UI-06, TRADE-01 | `feat/ui-management-flows` 已补 seller 页面权限拒绝、友好报价信息、接受确认与错误反馈；Repository/原子成交回归通过，待 PR 合并及最终双账号验收 |
...
| TRADE-08 | IN PROGRESS | 完成 ManagementActivity Tab 切换 | TRADE-06, TRADE-07 | `feat/ui-management-flows` 已完成 Tab/Adapter 切换、onResume 刷新、分 Tab 空状态和友好状态显示；API 36 空状态设备验收与全量回归通过，待 PR 合并 |
...
| INT-03 | IN PROGRESS | 统一错误、空状态和 Snackbar/Dialog 行为 | INT-01 | 登录/注册常见错误已迁入 string resources，重复昵称和 WhatsApp 格式使用字段错误；其余商品/管理页面随最终 E2E 检查 |
...
1. PR #3 至 #8 的认证、商品、报价、成交、schema、发布/编辑和详情页任务已合并；对应任务已更新为 `DONE`。
2. `UI-06/UI-07/TRADE-04/TRADE-08`：当前 `feat/ui-management-flows` 已完成报价弹窗、报价审核、My Listings 和 My Activity 展示整理；构建、Lint、45 个仪器测试和空状态设备截图通过，待 PR 合并。
3. `INT-01`：本批合并后从清空 App Data 开始跑 Alice/Bob 主流程，覆盖普通报价、Buy Now、卖家接受、双方状态和联系方式。
4. `INT-02/INT-03`：只修复主流程中实际发现的生命周期、返回栈、刷新和用户反馈问题，不扩展商用级异常处理。
