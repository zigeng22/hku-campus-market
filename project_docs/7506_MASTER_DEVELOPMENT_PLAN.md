...
4. 创建独立短分支；一次对话可在同一集成分支顺序完成多个任务 ID，但每个任务必须独立实现、验证和记录，且不得越过未通过的前置任务。
...
这一节是多人能否顺利合并的关键。`CONTRACT-01` 合并后即冻结；任何修改都必须由明确任务授权，并同时更新调用方、测试和本文件。契约版本 1.1 于 2026-07-22 经 `TRADE-06/CONTRACT-01` 授权，将 `ItemCard.status/offerCount` 正式纳入必填字段。2026-07-22 的 Enhancement Round 1 经用户明确授权，增加账户更新、Offer 拒绝和 Demo 图片修复方法；数据库字段、模型字段、Intent extra 和状态常量未变化，详见 `project_docs/7506_ENHANCEMENT_DEVELOPMENT_LOG.md`。
...
| `ItemCard` | 首页/My Listings 需要的 itemId、name、priceCents、imageUri、category、sellerNickname、status、offerCount；两个查询入口均返回真实状态与 Pending 报价数 |
...
- 业务数据保存在 App 私有目录的 SQLite 文件 `databases/hku_campus_market.db`，包含 `users`、`items`、`offers` 和 `trade_transactions` 四张表。
- 登录状态单独保存在 SharedPreferences 文件 `shared_prefs/market_session.xml`，只记录当前 `userId`；密码以 salt + PBKDF2 hash 存在 `users` 表中，不保存明文。
- 商品图片本身不复制进数据库；数据库只保存系统文件选择器返回的持久化 URI，读取失败时显示占位图。
- 正常关闭 App、强制停止、重启手机、重新打开 Android Studio，以及使用同一 applicationId 覆盖安装新版 APK，均不会清空数据。
- Android Studio/系统设置中的 Clear App Data、`adb shell pm clear`、卸载 App，或测试框架卸载测试 APK，会删除 SQLite 和 SharedPreferences；重新安装后从空数据开始。
- 当前项目没有云端同步，因此不同模拟器/手机各有独立数据库，换设备不会自动带出原设备账号和商品。
...
- 默认一个分支处理一个任务编号；经小组明确同意，一次 Agent 对话可使用一个集成分支顺序处理多个相邻任务，但每项必须有独立任务 ID、测试证据和可审查的改动范围。
...
| AUTH-04 | DONE | 实现登录/退出流程 | AUTH-01, AUTH-02, UI-02 | 登录、退出和清栈已连接；Repository/路由测试通过，2026-07-22 API 36 Alice/Bob 多次账号切换及强制重启 session 恢复验收通过 |
...
| UI-04 | DONE | Post/Edit Item XML | CONTRACT-02, UI-01 | PR #8 已于 2026-07-22 合并到 `main`（merge `0e99010`）；响应式滚动表单、固定保存区、图片预览、字段级错误和返回工具栏完成，411x914 实际发布/编辑与类别回填通过 |
| UI-05 | DONE | Item Detail XML 与买卖双方 action group | CONTRACT-02, UI-01 | PR #8 已合并；商品图、类别/价格/卖家/描述层级、固定 action 区和非 ACTIVE 提示完成，411x914 卖家详情实测无重叠 |
...
| ITEM-04 | DONE | 实现 HomeActivity 和 ItemAdapter | UI-03, ITEM-03, AUTH-04 | PR #8 合并后已完成 onResume、搜索、空状态、友好类别和详情导航；API 36 空数据及真实商品设备验收通过 |
| ITEM-05 | DONE | 实现 PostEditItemActivity 表单与金额转换 | UI-04, ITEM-02 | PR #8 已合并；发布/编辑实机流程、数据库类别代码回填、价格两位小数、字段错误和可见文本资源化均通过 |
| ITEM-06 | DONE | 实现相册选择和 URI 持久化 | ITEM-05 | PR #6 已于 2026-07-22 合并到 `main`（merge `b5309a2`）；持久权限异常与失效 URI 统一回退占位图，3 个 URI 测试及全量回归通过 |
| ITEM-07 | DONE | 实现 ItemDetailActivity 身份视图 | UI-05, ITEM-02 | PR #8 合并后 seller/buyer action、编辑、删除和 offer 导航均已连接；卖家设备流程及 `ItemDetailOfferFlowTest` 买家报价流程通过 |
...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| UI-06 | DONE | Make Offer Dialog 与 row_offer XML | UI-01, UI-05 | PR #9 已于 2026-07-22 合并到 `main`（merge `579168c`）；HKD 金额输入、报价类型/日期/状态和 Pending Accept 布局完成，编译、Lint 与 API 36 全量仪器测试通过 |
| UI-07 | DONE | Management/Listing/Participation layouts | UI-01, CONTRACT-02 | PR #9 已合并；两个 Tab、三种 row、分 Tab 空状态、商品图与成交联系方式布局完成，1080x2400 空状态设备截图通过 |
...
| TRADE-04 | DONE | 实现 seller offer list 查询与 OfferReviewActivity | UI-06, TRADE-01 | PR #9 已合并；seller 页面权限拒绝、友好报价信息、接受确认与错误反馈完成；2026-07-22 Alice 接受 Bob 报价的 API 36 设备流程通过 |
...
| TRADE-08 | DONE | 完成 ManagementActivity Tab 切换 | TRADE-06, TRADE-07 | PR #9 已合并；Tab/Adapter 切换、onResume 刷新、分 Tab 空状态和友好状态显示完成；Alice/Bob 的 My Listings/My Activity 设备流程通过 |
...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| INT-01 | DONE | 从干净安装跑 Alice/Bob 完整主流程 | Phase 1-3 | PR #10 已于 2026-07-22 合并到 `main`（merge `87dbaf9`）；API 36 从清空数据完成注册、发布、普通报价、Pending、卖家接受、首页下架、Deal confirmed/联系人及重启持久化 |
| INT-02 | DONE | 修复 Activity 生命周期、返回栈和 onResume 刷新 | INT-01 | PR #10 已合并；成交后 Offer Review、Item Detail、My Listings、Home 逐层返回均自动刷新，旧详情不再显示购买/卖家操作 |
| INT-03 | DONE | 统一错误、空状态和 Snackbar/Dialog 行为 | INT-01 | PR #10 已合并；类别下拉稳定展开，成交状态显示 `Deal confirmed`，登录/注册、商品、报价和管理中心普通流程反馈完成 |
| UI-08 | TODO | 视觉一致性和响应式检查 | INT-01 | 两种手机尺寸截图无重叠、截断、不可点击控件 |
| QA-01 | TODO | 全量手工测试矩阵 | INT-01 | 第 14 节所有 P0 用例有 Pass/Fail 与修复链接 |
| QA-02 | DONE | 单元/仪器测试回归和完整构建 | QA-01 | PR #10 已合并；assembleDebug、testDebugUnitTest、lintDebug 和 API 36 connectedDebugAndroidTest 均通过，45/45 仪器测试成功 |
| DEMO-01 | DONE | Seed demo data 或稳定测试账号方案 | INT-01 | PR #11 已于 2026-07-22 合并到 `main`（merge `30f8609`）；后续增强为 Alice/Bob/Carol/David、每人 3-4 个商品、共 14 个商品/图片和跨用户 Offer |
...
| ID | 状态 | 任务 | 依赖 | 交付物与验收 |
|---|---|---|---|---|
| DOC-01 | DONE | README | INT-01 | PR #11 已合并；README 覆盖功能、环境、构建运行、架构、4 个演示账号、本地数据、测试、限制和图片来源；最终提交前仍由 DOC-03 做 attribution 总审计 |
...
### 14.4 Debug 演示账号与预设数据

Debug APK 的 Login 页显示 `Prepare demo data`，Release APK 不显示且不会自动写入演示数据。首次点击后会准备以下本地账号，重复点击不会重复插入：

| 账号 | 密码 | 用途 |
|---|---|---|
| `AliceDemo` | `demo123` | 买家/卖家，WhatsApp `91234567`，4 个发布 |
| `BobDemo` | `demo123` | 买家/卖家，WhatsApp `92345678`，4 个发布 |
| `CarolDemo` | `demo123` | 买家/卖家，WhatsApp `93456789`，3 个发布 |
| `DavidDemo` | `demo123` | 买家/卖家，WhatsApp `94567890`，3 个发布 |

预设场景：

- `Java Programming Textbook`：ACTIVE，Bob 有 HK$100 的 Pending 普通报价，Alice 可在 My Listings 接受。
- `Scientific Calculator`：SOLD，Bob 的 My Activity 显示 Deal confirmed 和 Alice WhatsApp。
- 另有 12 个 ACTIVE 商品覆盖 Books、Electronics、Furniture、Daily Goods 和 Others；其中部分带 Pending Offer，部分只用于丰富首页和搜索。
- 14 张图片写入 Android 10+ 的 `Pictures/HKU Campus Market` 相册，首页初始显示 13 个 ACTIVE 商品。
- 点击准备按钮后登录表单自动填入 Alice；切换 Bob 时只需输入以上固定账号密码。
- 需要从头重建场景时，手动 Clear App Data，再打开 Debug APK 点击一次按钮；不要在正式演示中运行全量仪器测试，因为测试任务可能卸载 App 并清除本地演示数据。
...
1. PR #3 至 #10 的全部 P0 功能、集成和回归任务已合并；`INT-01/02/03` 与 `QA-02` 已更新为 `DONE`。
2. `DEMO-01/DOC-01` 已由 PR #11 合并完成；后续功能完善统一记录在 `7506_ENHANCEMENT_DEVELOPMENT_LOG.md`。
3. `feat/demo-catalog-docs` 将 Demo 扩展为 14 个商品/图片，并新增 `7506_PROJECT_STRUCTURE_AND_TESTING_GUIDE.md` 供组员理解结构和对照测试。
4. 下一批执行 `UI-08/QA-01`：只检查主要手机尺寸和第 14 节 P0 手工矩阵，修复实际可见问题。
5. 之后处理报告、视频脚本和最终提交材料，不新增后台、云服务或商用级功能。
